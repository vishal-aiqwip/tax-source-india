...
for (const name of Object.keys(TARGETS)) {
  try {
    await access(path.join(SRC, name));
  } catch {
    try {
      await copyFile(path.join(OUT, name), path.join(SRC, name));
    } catch {
      /* absent */
    }
  }
...
for (const name of await readdir(SRC)) {
  ...
  const outName =
    target.format === "webp" ? name.replace(/\.(png|jpe?g)$/, ".webp") : name;
  ...
  if (target.format !== "webp" && buf.length >= wasSize) {
    ...
    rows.push(
      `  ${name.padEnd(17)} ${kb(wasSize)} -> ${kb(wasSize)}  (kept original, re-encode was larger)`,
    );
    before += wasSize;
    after += wasSize;
    continue;
  ...
  after += nowSize;
  rows.push(
    `  ${name.padEnd(17)} ${kb(wasSize)} -> ${kb(nowSize)}  ${outName.padEnd(18)} ${meta.width}x${meta.height}`,
  );
}

// A real favicon — the 253 KB logo-mark.png was doing this job.
await sharp(path.join(SRC, "logo-mark.png"))
  .resize(48, 48)
...
console.log(
  `  ${"TOTAL".padEnd(17)} ${kb(before)} -> ${kb(after)}   (${(100 - (after / before) * 100).toFixed(0)}% smaller)`,
);
for (const extra of [
  "favicon-48.png",
  ...
  console.log(
    `  + ${extra.padEnd(21)} ${kb((await stat(path.join(OUT, extra))).size)}`,
  );
}
